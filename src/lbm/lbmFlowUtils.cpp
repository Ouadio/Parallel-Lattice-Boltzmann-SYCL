...
void initialize_macroscopic_variables(const LBMParams &params, real_t *rho,
                                      real_t *ux, real_t *uy, sycl::queue &q,
  ...
  evs.emplace_back(q.submit(
      [&](sycl::handler &cgh) { cgh.fill<real_t>(rho, fillRho, nx * ny); }));

  // uy is zero everywhere (right here/now only): fill 0
  const real_t fillUy = 0.0;

  evs.emplace_back(q.submit(
      [&](sycl::handler &cgh) { cgh.fill<real_t>(uy, fillUy, nx * ny); }));
...
// ======================================================
void initialize_equilibrium(const LBMParams &params, const real_t *v,
                            const real_t *t, const real_t *rho,
                            const real_t *ux, const real_t *uy, real_t *fin,
                            sycl::queue &q, sycl::event &ev) {
  ...
  ev = q.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(sycl::range<2>{ny, nx}, [=](sycl::id<2> id) {
      ...
      for (int ipop = 0; ipop < npop; ++ipop) {
        ...
        fin[index + ipop * nx * ny] =
            rho[index] * t[ipop] * (1 + cu + 0.5 * cu * cu - usqr);
      }
    });
  });
} // init equilibrium
...
// ======================================================
void border_outflow(const LBMParams &params, real_t *fin, sycl::queue &q,
                    sycl::event &ev) {
...
// ======================================================
void macroscopic(const LBMParams &params, const real_t *v, const real_t *fin,
                 real_t *rho, real_t *ux, real_t *uy, sycl::queue &q,
                 sycl::event &ev) {
...
// ======================================================
void border_inflow(const LBMParams &params, const real_t *fin, real_t *rho,
                   real_t *ux, real_t *uy, sycl::queue &q, sycl::event &ev) {
  ...
  ev = q.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(sycl::range<1>{ny}, [=](sycl::id<1> j) {
      ...
      rho[index] = 1 / (1 - ux[index]) *
                   (fin[index + 3 * nxny] + fin[index + 4 * nxny] +
                    fin[index + 5 * nxny] +
                    2 * (fin[index + 6 * nxny] + fin[index + 7 * nxny] +
                         fin[index + 8 * nxny]));
    });
  });
...
// ======================================================
void equilibrium(const LBMParams &params, const real_t *v, const real_t *t,
                 const real_t *rho, const real_t *ux, const real_t *uy,
                 real_t *feq, sycl::queue &q, sycl::event &ev) {
  ...
  ev = q.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(sycl::range<2>{ny, nx}, [=](sycl::id<2> id) {
      ...
      for (int ipop = 0; ipop < npop; ++ipop) {
        real_t cu = 3 * (v[ipop * 2] * ux[index] + v[ipop * 2 + 1] * uy[index]);

        feq[index + ipop * nx * ny] =
            rho[index] * t[ipop] * (1 + cu + 0.5 * cu * cu - usqr);
      }
    });
  });
...
// ======================================================
void update_fin_inflow(const LBMParams &params, const real_t *feq, real_t *fin,
                       sycl::queue &q, sycl::event &ev) {
  ...
  ev = q.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(sycl::range<1>{ny}, [=](sycl::id<1> j) {
      ...
      fin[index + 0 * nxny] =
          feq[index + 0 * nxny] + fin[index + 8 * nxny] - feq[index + 8 * nxny];
      fin[index + 1 * nxny] =
          feq[index + 1 * nxny] + fin[index + 7 * nxny] - feq[index + 7 * nxny];
      fin[index + 2 * nxny] =
          feq[index + 2 * nxny] + fin[index + 6 * nxny] - feq[index + 6 * nxny];
    });
  });
...
// ======================================================
void compute_collision(const LBMParams &params, const real_t *fin,
                       const real_t *feq, real_t *fout, sycl::queue &q,
                       sycl::event &ev) {
...
// ======================================================
void update_obstacle(const LBMParams &params, const real_t *fin,
                     const uint8_t *obstacle, real_t *fout, sycl::queue &q,
                     sycl::event &ev) {
...
// ======================================================
void streaming(const LBMParams &params, const real_t *v, const real_t *fout,
               real_t *fin, sycl::queue &q, sycl::event &ev) {
...
void prepare_png_output(const LBMParams &params, const real_t *ux,
                        const real_t *uy, real_t *u2, unsigned char *img,
                        unsigned char *imgH, real_t *max_value,
                        real_t *min_value, sycl::queue &q,
                        std::vector<sycl::event> &evs) {
  ...
  evs.emplace_back(q.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(sycl::range<2>{ny, nx}, [=](sycl::id<2> id) {
      ...
      img[0 + 4 * i + 4 * nx * j] = 255 - value;
      img[1 + 4 * i + 4 * nx * j] = 255 - value;
      img[2 + 4 * i + 4 * nx * j] = 255 - value;
  ...
  evs.at(2).wait(
      {evs.at(0),
       evs.at(
           1)}); // Present kernel requires the 2 previous events to be complete
  evs.emplace_back(q.submit([&](sycl::handler &cgh) {
    cgh.memcpy(imgH, img, nx * ny * 4 * sizeof(unsigned char));
  }));
  evs.at(3).wait({evs.at(2)});

} // LBMSolver::prepare_png_output

void output_png_parallel(int iTime, int nx, int ny, const unsigned char *img) {
